# SEO 사이트 분석기

URL을 입력하면 콘텐츠 SEO / 테크니컬 SEO / 검색엔진 친화도 / 속도 최적화 / 보안 권장사항을 점검해주고, 브랜드 키워드로 네이버 블로그·뉴스·카페 발행 현황과 검색량·연관 키워드·검색 트렌드를 조회할 수 있는 웹사이트입니다.

## 배포 방법 (요약)

1. 이 폴더 전체를 GitHub 새 저장소(New repository)에 업로드합니다. ("Add file" → "Upload files"로 드래그 앤 드롭하면 됩니다. `node_modules` 폴더는 없으니 그대로 올리면 됩니다.)
2. https://vercel.com 에서 GitHub 계정으로 로그인 후 "Add New..." → "Project" → 방금 만든 저장소 선택 → "Deploy" 클릭.
3. (브랜드 콘텐츠 발행 현황 / 키워드 분석 기능을 쓰려면) 아래 환경변수 설정 안내대로 Vercel에 네이버 API 키를 등록합니다. 3그룹 중 필요한 것만 등록해도 되고, 등록 안 한 그룹은 해당 기능에서만 안내 문구가 표시돼요.
4. 몇 분 후 생성되는 주소로 접속하면 바로 사용할 수 있습니다.

## 환경변수 설정 — 3개의 서로 다른 네이버 API

이 사이트는 성격이 다른 네이버 API 3가지를 사용해요. **서로 발급 방법이 완전히 다르니** 그룹별로 따로 신청해야 합니다.

| 그룹 | 사용하는 기능 | 발급처 |
|---|---|---|
| ① 검색 API (API HUB) | 브랜드 콘텐츠 발행 현황, 키워드 분석의 콘텐츠 발행량 | 네이버 클라우드 플랫폼(NCP) |
| ② 검색광고 API | 키워드 분석의 월간 검색량 · 연관 키워드 | searchad.naver.com |
| ③ 데이터랩 API (선택) | 키워드 분석의 검색량 트렌드 · 월별/요일별 검색 비율 | developers.naver.com |

### ① 검색 API (NAVER API HUB)

브랜드 키워드로 블로그/뉴스/카페 발행 현황을 조회하는 기능은 네이버 검색 API 키가 있어야 동작합니다.

⚠️ 네이버가 검색 API를 예전 "네이버 개발자센터"에서 **NAVER API HUB(네이버 클라우드 플랫폼)** 로 옮겼기 때문에, 개발자센터(`developers.naver.com`)에서는 더 이상 "검색" API를 신규로 등록할 수 없습니다. 아래처럼 API HUB에서 발급받아야 합니다.

1. https://www.ncloud.com/product/applicationService/naverApiHub 에서 네이버 클라우드 플랫폼(NCP) 계정으로 로그인/가입합니다. (일반 네이버 아이디와는 별도 가입입니다.)
2. NAVER API HUB 상품을 이용 신청하고, Application 생성 화면에서 블로그/뉴스/카페글 검색 API를 선택합니다.
3. 발급되는 **Key ID**와 **Key(Secret)** 값을 복사해둡니다.
4. Vercel 프로젝트 → Settings → Environment Variables 에서 아래 두 개를 추가합니다.
   - `NAVER_CLIENT_ID` = 위에서 복사한 Key ID
   - `NAVER_CLIENT_SECRET` = 위에서 복사한 Key(Secret)
   - Environment는 Production/Preview/Development 전부 체크해두면 편해요.
5. 환경변수를 추가한 뒤에는 Deployments 탭 → 가장 최근 배포 옆 "..." 메뉴 → **Redeploy**를 눌러야 반영됩니다. (환경변수만 추가하고 Redeploy를 안 하면 이전 값 그대로 동작해요.)

키를 등록하지 않아도 사이트 자체는 정상 동작하며, 브랜드 콘텐츠 분석 시도 시에만 안내 메시지가 표시됩니다.

### ② 검색광고 API (키워드 분석 — 검색량/연관 키워드)

1. https://searchad.naver.com 에 로그인합니다. (검색광고 계정이 없다면 먼저 만들어야 해요. 별도 심사 없이 약관 동의만으로 가입돼요.)
2. 상단 메뉴 **도구 → API 사용 관리**로 들어가 약관에 동의하면 즉시 발급됩니다.
3. 발급되는 **API License(Key)**, **SECRET KEY**, 그리고 화면에 표시되는 **CUSTOMER ID**(광고주 계정 번호, 6~8자리 숫자)를 복사해둡니다.
4. Vercel 프로젝트 → Settings → Environment Variables 에서 아래 세 개를 추가합니다.
   - `NAVER_AD_API_KEY` = API License(Key)
   - `NAVER_AD_SECRET_KEY` = SECRET KEY
   - `NAVER_AD_CUSTOMER_ID` = CUSTOMER ID
5. 추가 후 Redeploy를 눌러야 반영됩니다.

이 키가 없으면 키워드 분석 페이지에서 "월간 검색량"과 "연관 키워드" 부분만 안내 문구로 표시되고, 나머지 기능은 정상 동작해요.

### ③ 데이터랩 API (키워드 분석 — 검색량 트렌드, 선택 사항)

1. https://developers.naver.com/apps 에서 애플리케이션을 새로 등록합니다. (① API HUB 키와는 별개로, 여기서는 예전 방식 그대로 신청해요.)
2. 사용 API 목록에서 **데이터랩(검색어트렌드)** 를 선택해 이용 신청합니다. ※ 신청 화면 구성은 네이버 정책에 따라 바뀔 수 있어요. 만약 목록에 데이터랩 항목이 보이지 않는다면 이 기능은 현재 신규 발급이 막힌 것이니, 이 그룹은 비워두고 나머지 기능만 사용하시면 됩니다.
3. 발급된 **Client ID**, **Client Secret**을 복사해둡니다.
4. Vercel 환경변수에 아래 두 개를 추가합니다.
   - `NAVER_DATALAB_CLIENT_ID` = Client ID
   - `NAVER_DATALAB_CLIENT_SECRET` = Client Secret
5. 추가 후 Redeploy를 눌러야 반영됩니다.

이 키가 없으면 "검색량 트렌드", "월별/요일별 검색 비율" 차트만 안내 문구로 표시되고, 나머지 기능은 정상 동작해요.

## 키워드 분석 기능에서 제공하지 않는 항목

다음 항목들은 네이버 공식 API로 가져올 수 없어서 이번 기능에서 제외했습니다.

- **인기 급상승 키워드** — 네이버가 실시간 급상승 검색어 서비스를 2021년에 종료한 뒤로 관련 공식 API가 없습니다.
- **연령별 · 성별 검색 비율** — 데이터랩 API는 쇼핑 카테고리에 한해서만 연령/성별 통계를 제공하고, 임의의 키워드에는 제공하지 않습니다.
- **검색결과 웹사이트 영역 TOP10 콘텐츠/사이트 정보**, **PC/모바일 섹션 배치 순서** — 네이버 검색결과 화면 자체를 가져오는 공식 API가 없고, 실제 검색결과 페이지를 직접 스크래핑해야 하는데 이는 네이버 이용약관 위반 소지가 있고 화면 구조가 바뀌면 바로 깨지기 때문에 제외했습니다.

## 로컬에서 실행하고 싶다면 (선택, 개발자용)

```bash
npm install
npm run dev
```

http://localhost:3000 에서 확인할 수 있습니다.
