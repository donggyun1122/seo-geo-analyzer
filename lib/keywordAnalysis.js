...
// - 검색량 트렌드 / 월별·요일별 검색비율     : 네이버 API HUB의 Search Trend(데이터랩) API
//
...
//
// ※ 연령별/성별 검색 비율은 일부러 뺐습니다. Search Trend의 ratio는 "한 번의 요청 안에서"
// 최고값을 100으로 정규화한 상대값이라, 연령대나 성별을 나눠 각각 따로 호출한 뒤 그 ratio를
// 합산·비교하면 서로 다른 기준(100)을 더하는 셈이라 수치가 아예 틀리게 나옵니다. 이 API로는
// 여러 그룹을 같은 기준으로 비교하는 방법이 없어서 정확하게 계산할 수 없는 항목이라 제외했어요.
...
// 월별 트렌드 / 요일별 비율은 서로 완전히 독립된 API 호출이라, 하나가 실패해도
// (예: 순간적인 호출 한도 초과) 나머지는 그대로 보여줄 수 있도록 각각 따로 처리합니다.
async function fetchTrend(keyword) {
  if (!isDatalabConfigured()) {
    const notConfigured = { available: false, reason: "NOT_CONFIGURED" };
    return { monthly: notConfigured, weekday: notConfigured };
  }

  const [monthlyResult, weekdayResult] = await Promise.allSettled([
