...
async function callDatalabOnce(keyword, { startDate, endDate, timeUnit, device, gender, ages }, clientId, clientSecret) {
  const body = {
    startDate,
    endDate,
    timeUnit,
...
// 여러 요청을 동시에 쏟아내면 NCP APIGW의 초당 호출 한도(rate limit)에 걸릴 수 있어,
// 약간의 간격을 두고 순차적으로 호출합니다. 하나가 실패해도 나머지는 계속 진행돼요.
async function runStaggered(tasks, delayMs = 150) {
  const results = [];
...
export async function fetchAgeGenderRatio(keyword) {
  ...
  // 연령대 6개 + 성별 2개 = 총 8건을 한꺼번에 쏘지 않고 약간의 간격을 두고 순차 호출합니다.
  const tasks = [
    ...AGE_GROUPS.map((g) => () => callDatalab(keyword, { startDate, endDate, timeUnit: "month", ages: g.codes })),
  ...
  const settled = await runStaggered(tasks);
  ...
  const ageResults = settled.slice(0, AGE_GROUPS.length).map((r) => r.value);
  const maleSeries = settled[AGE_GROUPS.length].value;
  const femaleSeries = settled[AGE_GROUPS.length + 1].value;
