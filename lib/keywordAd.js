...
// 월간 검색량(PC/모바일), 연관 키워드, 광고 경쟁정도를 제공합니다.
...
// 반환된 배열의 각 항목: { keyword, isSeed, pcCount, mobileCount, totalCount, pcLabel, mobileLabel, compIdx }
export async function fetchRelatedKeywords(rawKeyword) {
  ...
  if (!res.ok) {
    ...
    const err = new Error(
      `네이버 검색광고 API 호출 실패 (status ${res.status})${bodyText ? `: ${bodyText.slice(0, 200)}` : ""}`
    );
