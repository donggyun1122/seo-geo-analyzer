...
// recommendation is only shown to the user when the check fails (pass === false)
function makeCheck(id, label, weight, pass, detail, recommendation) {
  return { id, label, weight, pass: !!pass, detail, recommendation: pass ? null : recommendation };
}
...
async function analyzeUrl(rawUrl) {
  ...
  const searchFriendliness = categoryScore([
    ...
    makeCheck(
      ...
      "og:title, og:description, og:image 태그를 추가해 SNS 공유 시 미리보기가 제대로 표시되게 하세요."
    ),
    makeCheck(
      "twitter_card",
      "Twitter Card 태그",
  ...
  // ============ TOP 5 우선 개선 사항 ============
  // 카테고리 전체를 합쳐서, 실패한 항목 중 가중치(중요도)가 높은 순으로 5개를 뽑는다.
  const taggedChecks = [
