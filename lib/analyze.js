...
// ---- 브랜드 키워드 자동 추정 (og:site_name → JSON-LD → title → 도메인명 순) ----
const TWO_PART_TLDS = new Set([
  "co.kr", "or.kr", "ne.kr", "go.kr", "pe.kr", "re.kr", "ac.kr",
  "co.uk", "org.uk", "ac.uk",
  "com.au", "com.cn", "com.tw", "com.br", "co.jp", "ne.jp",
]);

function extractDomainGuess(finalUrl) {
  try {
...
// JSON-LD 안의 Organization/WebSite name 값을 찾습니다. 파싱 실패는 조용히 건너뜁니다.
function extractJsonLdBrandName($) {
  ...
  for (const el of scripts) {
    ...
    for (const node of roots) {
      ...
      for (const item of items) {
        ...
        if (/organization|website/i.test(typeStr) && typeof item.name === "string" && item.name.trim()) {
...
// <title>은 "브랜드명 | 카테고리" 형태가 많아 구분자로 나눈 뒤 가장 짧은 조각을 추정치로 사용합니다.
function extractTitleGuess(titleText) {
  if (!titleText) return "";
...
function guessBrandKeyword({ ogSiteName, $, titleText, finalUrl }) {
  if (ogSiteName && ogSiteName.trim()) return ogSiteName.trim();
  const jsonLdName = extractJsonLdBrandName($);
  if (jsonLdName) return jsonLdName;
  const titleGuess = extractTitleGuess(titleText);
  if (titleGuess) return titleGuess;
  return extractDomainGuess(finalUrl);
}
...
async function analyzeUrl(rawUrl) {
  ...
  const suggestedBrandKeyword = guessBrandKeyword({ ogSiteName, $, titleText, finalUrl });
